    ...
    <>
      <div >
        ...
        <form onSubmit={(e) => e.preventDefault()} className='w-3/12  absolute  p-12  bg-black my-36 mx-auto right-0 left-0 text-white rounded-lg bg-opacity-85  '>
          ...
          <input type="text" ref={email} placeholder='Email ID' className='p-4 my-4 w-full rounded-lg bg-gray-700' />

          <input type="text" ref={password}
            placeholder='Password' className='p-4 my-4 w-full rounded-lg bg-gray-700' />

          <p className='text-red-500 font-bold text-lg py-2'>{errormessage}</p>

          <button className='p-4 my-6 bg-red-700 rounded-lg w-full' onClick={handleButtonClick}>{isSignInForm ? "Sign In" : "Login"}</button>
